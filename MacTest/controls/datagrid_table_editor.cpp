...
#ifndef GacOSX_datagrid_table_editor_h
#define GacOSX_datagrid_table_editor_h
...
private:
    ...
    void Initialize()
    {
        ...
        FOREACH(WString, file, files)
        {
            ...
            stringGrid->Grids().SetGridString(row, 0, osx::GetFileDisplayName(fullPath));
            stringGrid->Grids().SetGridString(row, 1, osx::GetFileModificationTimeString(fullPath));
            stringGrid->Grids().SetGridString(row, 2, osx::GetFileDisplayType(fullPath));
            stringGrid->Grids().SetGridString(row, 3, GetFileSizeString(GetFileSize(fullPath)));
        }
    }
public:
    TableEditorWindow()
